...
export class NotificationsService {
  ...
  subscribe(userId: string, role: string): { obs: Observable<MessageEvent>; close: () => void } {
  ...
  async create(senderId: string, dto: CreateNotificationDto) {
    ...
    const n = await this.prisma.notifications.create({
      data: { id, senderId, title: dto.title, body: dto.body, target: dto.target },
      select: { id: true, title: true, body: true, target: true, createdAt: true },
    });
