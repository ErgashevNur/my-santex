...
export class UsersController {
  ...
  @Get()
  findAll(@CurrentUser() user: JwtUser, @Query('storeId') storeId?: string) {
    // Super Admin: query param bilan istalgan do'kon xodimlarini ko'ra oladi
    const targetStore =
      user.role === UserRole.SUPER_ADMIN && storeId
        ? storeId
        : (user.storeId ?? '');
  ...
  toggleActive(@CurrentUser() user: JwtUser, @Param('id') id: string) {
